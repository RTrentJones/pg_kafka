...
( cd "$HERE" && BROKER="${PG_KAFKA_BROKER:-localhost:9092}" LABEL=pg_kafka \
  OUT="$WORK/bench-pg_kafka.json" node bench.mjs ) || echo "::warning::pg_kafka bench failed"

echo "== bench: real broker =="
( cd "$HERE" && BROKER="${REAL_BROKER:-localhost:9093}" LABEL=realbroker \
  OUT="$WORK/bench-realbroker.json" node bench.mjs ) || echo "::warning::real-broker bench failed"

echo "== bench: raw INSERT =="
( cd "$HERE" && OUT="$WORK/bench-rawinsert.json" node raw_insert.mjs ) || echo "::warning::raw-insert bench failed"
